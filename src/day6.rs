...
fn part_one(contents: &Vec<String>) -> i32 {
    ...
    for c in contents {
        ...
        match commands[0].as_ref() {
            "toggle" => {
                let start: Vec<usize> =
                    commands[1].split(",").map(|i| i.parse().unwrap()).collect();
                let end: Vec<usize> = commands[3]
                    .split(",")
            ...
            "turn" => {
                if commands[1] == "off" {
                    let start: Vec<usize> =
                        commands[2].split(",").map(|i| i.parse().unwrap()).collect();
                    let end: Vec<usize> = commands[4]
                        .split(",")
                        .map(|i| i.trim().parse().unwrap())
                        .collect();
                    for x in start[0]..end[0] + 1 {
...
fn main() {
    let home = env::current_dir().expect("Where's my current path?");
    let filepath = home.join("src/data/day6.txt");
